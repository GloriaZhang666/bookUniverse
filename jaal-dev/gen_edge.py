...
def gen_edge_csv(columns):
    ...
    inputName =os.path.join(this_dir, "got", "got_node_df.csv")
    outputName = os.path.join(this_dir, "got", "got_edge_df.csv")
    # read node csv data to a dataframe
    df = pd.read_csv(inputName)

    # fill blank with 0
    df.fillna(0, inplace=True)
    # output full df to csv
    ...
    if 'All' in columns:
        ...
        j = 0
        # loop through ids of books df
        for id in df['id'].values:
            # set from
            edgeDf.at[j, "from"] = id
            if j > 0:
                # if current index > 0, then set last to
                edgeDf.at[j - 1, "to"] = id
            # index++
            j += 1
